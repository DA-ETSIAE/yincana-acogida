const estaciones = {
    ...
    "Aula Miguel Ángel Barcala": ['¿Quién fue Miguel Ángel Barcala?','¿De qué año a que año fue director de la EUITA Miguel Ángel Barcala?'],
...
function sacarPregunta() {
  const estacion = new URLSearchParams(document.location.search).get(
    "estacion",
  );

  if (!estacion || !estaciones.hasOwnProperty(estacion)) {
    window.location.replace(
      "https://aeroespacial.da.upm.es/yincana/?estacion=Avión",
    );
    return;
  }

  const preguntas = estaciones[estacion];
  const siguiente_estacion =
    ordenEstaciones[ordenEstaciones.indexOf(estacion) + 1];
  ...
  if (siguiente_estacion != null) {
    ...
    document.getElementById("estacion_next").innerHTML =
      "<b>Próxima Estación</b>: " + "<i>" + ordenEstaciones[0] + "</i>";
  }
}
